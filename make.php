...
//========================
	...
	infrajs::isAdd('check',function(&$layer){//может быть у любого слоя в том числе и у не iswork, и когда нет старого значения
		...
		if(!infrajs::isWork($layer))return false;//Нет сохранённого результата, и слой не в работе, если работа началась с infrajs.check(layer) и у layer есть родитель, который не в работе
...
//========================
	...
	infrajs::isAdd('show',function(&$layer){//Родитель скрывает ребёнка если у родителя нет опции что ветка остаётся целой
		...
		if(infrajs::isSaveBranch($layer['parent']))return;//Какой-то родитель таки не показывается, например пустой слой, теперь нужно узнать скрывает родитель свою ветку или нет
	...
	infrajs::isAdd('show',function(&$layer){
		//tpl depricated
		if(ext\tpl::onlyclient($layer))return;
