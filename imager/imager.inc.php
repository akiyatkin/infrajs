...
*/
	...
	function imager_getReal($src){
		...
		$path=$p[0];
		$query=$p[1];
		if(!$query)return infra_theme($path,'fn');
		if(preg_match("/theme\.php$/",$path)){
			return imager_getReal($query);
		}else if(preg_match("/imager\.php$/",$path)){
			$p=explode('&',$query);
			$obj=array();
	...
	function imager_type($src){
		...
		return infra_cache(array($src),'imager_type',function($src){
			...
			fclose($handle);
			if(preg_match("/JFIF/",$line))return 'jpeg';
			if(preg_match("/PNG/",$line))return 'png';
			if(preg_match("/GIF/",$line))return 'gif';
			if(preg_match("/Exif/",$line))return 'jpeg';
			if(preg_match("/Exif/",$line2))return 'jpeg';
			if(preg_match("/BM/",$line))return 'wbmp';
			return false;
	...
	function imager_mark($src,$type){
		...
		if($type=='gif')return;//Проблема прозрачности
		
		$info=&imager_readInfo($src);

		if(@$info['ignore'])return;//В изображении указано что не нужно делать водяной знак на нём
	...
	function imager_resize($src,$type,$w,$h,$crop=false,$top=false,$bottom=false){
		...
		if($w&&$h){
			...
			}else if($k_orig>$k){//ширины в оригинале больше чем в требуемом.. с учётом размеров высоты.
				if(!$crop){//Значит чтобы ничего не обрезать и быть в рамках меняем ширину а высота и так относительно меньше требуемой
					$h=false;//Значит высоту нужно высчитать отностительно ширины
				}else{//Ну а если нужно чтобы указанные размеры были полностью заполнены то ширину надо обрезать и равняться будем уже на высоту
					$d=$h/$height_orig;//Коэфициент на сколько изменяем оригинальный размер
					$dw = (($width_orig*$d - $w))/$d;
				}
			}else{
				if(!$crop){//Значит меняем ширину а высота и так относительно меньше требуемой
					$w=false;
				}else{//Ну а если обрезать, то высоту/ Ровняемся на ширину
		...
		if(!is_file($src_cache)||filemtime($src_cache)<filemtime($src)
			...
			if($type=='gif'){
				...
				if($transparentcolor==-1)$transparentcolor=255;
