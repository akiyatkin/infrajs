...
function infra_fire(&$obj,$clsfn,$argso=array()){
	global $infra;
	if($obj!==$infra) $clsfn=explode('.',$clsfn);
	else $clsfn=array($clsfn);
...
function infra_when(&$obj,$fn,$callback){//depricated, для классов не подходит
	$depot=&infra_fire_depot($obj,$fn);
...
function infra_wait(&$obj,$fn,$callback){//depricated, для классов не подходит
	$depot=&infra_fire_depot($obj,$fn);
	if($depot['evt']){
...
function infra_handle(&$obj,$fn,&$callback){//depricated, для классов не подходит
	$depot=infra_fire_depot($obj,$fn);
	if($depot['evt']){
		infra_fire_exec($depot,$callback);
	}
...
function infra_unlisten(&$obj,$fn,&$callback){
	$depot=infra_fire_depot($obj,$fn);
	foreach($depot['listen'] as &$cal){
		if($cal===$callback)unset($cal);
		break;
	}
}
function &infra_fire_depot(&$obj,$fn){
	$n='__infra_fire_depot__';
	if(!isset($obj[$n]))$obj[$n]=array();
	if(!isset($obj[$n][$fn]))$obj[$n][$fn]=array(//При повторном событии этот массив уже будет создан
