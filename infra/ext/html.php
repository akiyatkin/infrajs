...
function htmlGetUnick(){
	...
	if(!$htmlGetUnick_last_time)$htmlGetUnick_last_time=0;
	$t=time();
	while($t<=$htmlGetUnick_last_time)$t++;
...
function infra_html($html=null,$id=null){
	...
	if(!$infra_store_html)$infra_store_html;
	...
	$t='·';
	while(strpos($infra_store_html,$t)!==false){//Смотрим нет ли указанного символа в шаблоне, если нет то можно его использовать в качестве временной замены
		$t=htmlGetUnick();
	}
