...
function infra_json_encode($mix, $pretty = true)
{
    ...
    }//для 5.2.2 и всего что старее 5.2.4
    else {
...
//foreach($GLOBALS as $k=>&$v){
            ...
            $FROM_PHP = true;
            if (is_null($FROM_PHP_PLUGIN)) {
                $FROM_PHP_PLUGIN = true;
            } elseif ($FROM_PHP_PLUGIN) {
                $FROM_PHP_PLUGIN = true;
            } else {
                $FROM_PHP_PLUGIN = null;
            }//При следующем вложенном подключении установится уже в true
            ...
            if ($rrr !== 1 && !is_null($rrr)) {
                //в include небыло return.. он просто выполнился и всё
                $result = $rrr;
...
function infra_isphp($val = null)
{
    ...
    if (!is_null($val) && is_null($FROM_PHP_PLUGIN)) {
        //можно установить false если ещё небыло никаких установок.. если кто-то подключает в php через theme.php или тп... сброс в theme.php уже не сработает
        $FROM_PHP_PLUGIN = $val;
    } else {
