...
function &infra_template_stor(){
	...
	if(!isset($stor['template']))$stor['template']=array();
	if(!isset($stor['template']['cache']))$stor['template']['cache']=array();
	return $stor['template'];
}
function &infra_template_make($url,$tplempty='root'){
	$key=md5(print_r($url,true));
	$stor=&infra_template_stor();
	if(isset($stor['cache'][$key]))return $stor['cache'][$key];

	if(is_string($url))$template=infra_loadTEXT($url);
	else if(is_array($url))$template=$url[0];
...
function infra_template_exec(&$tpls,&$data,$tplroot='root',$dataroot=''){//Только тут нет conf
	if(is_null($tplroot))$tplroot='root';
	if(is_null($dataroot))$dataroot='';
...
function &infra_template_getPath(&$conf,$var){//dataroot это прощитанный путь до переменной в котором нет замен
	/*
	...
	infra_forr($var,function(&$conf,&$ar,&$v){//'[asdf,asdf,[asdf],asdf]'
		if(is_string($v)||is_int($v)){//name
		...
		}else if(is_array($v)&&is_string($v['orig'])){//name.name().name [name,{},name]
			global $infra_template_scope;
			//$t=array_merge($ar,$v);
			if($ar){//смутнопонимаемая ситуация... asdf().qewr().name после замены получаем zxcv.qewr().name потом tyui.name для того чтобы получить tyui нужно установить dataroot zxcv
			...
			if(!isset($infra_template_scope['zinsert']))$infra_template_scope['zinsert']=array();
...
function infra_template_getVar(&$conf,$var=array()){//dataroot это прощитанный путь до переменной в котором нет замен
	...
	if(is_null($var)){
		...
		if(@(string)$p[sizeof($p)-1]=='$key'){
			...
			if(is_null($value))$root=$right;//Афигенная ошибка. получена переменная и далее идём к шаблону переменной для которого нет, узнав об этом lastroot не сбивается и шаблон дальше загружается с переменной в lastroot {$indexOf(:asdf,:s)}{data:descr}{descr:}{}	
		}
	}
	return array(
...
function infra_template_getCommaVar(&$conf,&$d,$term=false){
	...
	if(@$d['fn']){
		...
		if(is_callable($func)){
			...
			return null;//что возвращается когда нет функции которую нужно вызвать
			/*if($term)return null;
			else return $d['orig'];*/
		}
	}else{
...
function infra_template_getValue(&$conf,&$d,$term=false){
	...
	if(@$d['cond']&&!isset($d['term'])){
		...
		if(is_null($var)||$var===false||$var===''||$var===0){//Пустой массив не false
...
function infra_template_getTpls(&$ar,$subtpl='root'){//subtpl - первый подшаблон с которого начинается если конкретно имя не указано
	$res=array();

	for($i=0;$i<sizeof($ar);$i++){
		if(is_array($ar[$i])&&isset($ar[$i]['template'])){//Если это шаблон
		...
		if(!isset($res[$subtpl]))$res[$subtpl]=array();
		$res[$subtpl][]=$ar[$i];
	}

		global $itn;
	foreach($res as $subtpl=>$v){//Удаляется последний символ в предыдущем подшаблонe
...
function infra_template_parseStaple($exp){
	...
	if(is_string($newexp)) $exp=$newexp;
	if(is_string($newexp)&&is_string($fn))$exp.=$fn;
	return $exp;
}
function infra_template_parseexp($exp,$term=false,$fnnow=null){// Приоритет () ? | & = ! : [] , .
	/*
...
function infra_template_parsevar($var){//Ищим скобки as.df[asdf[y.t]][qwer][ert]   asdf[asdf][asdf]
	if($var=='')return null; //Замен xinsert уже нет //asdf.asdf[asdf] На выходе ['asdf','asdf',['asdf']]
	$res=array();
...
$infra_template_scope=array(
	...
	'~length'=>function($obj=null){
		...
		if(is_array($obj)){
			return sizeof($obj);
		}if(is_string($obj))return strlen($obj);
		return 0;
	},
	'$inArray'=>function(){
		$args=func_get_args();
	...
	'~words'=>function($count,$one='',$two=null,$five=null){
		if(is_null($two))$two=$one;
		if(is_null($five))$five=$two;
		if(!$count)$count=0;
		if($count>20){
			$str=(string)$count;
	...
	'~multi'=>function(){
		$args=func_get_args();
		$n=1;
		for($i=0,$l=sizeof($args);$i<$l;$i++) $n*=$args[$i];
		return $n;
	},
